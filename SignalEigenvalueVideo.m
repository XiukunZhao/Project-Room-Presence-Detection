% This file is used to show a comparison of signal, eigenvalue, and video
% Input: Motion matrix and video data
% Output: Animation showing origenal video (without noise) and motion matrix
...
aviname = 'WalkInOutWithoutFan.avi';
...
for j = VideoLabelStart:1:VideoLabelEnd
    ...
    title('signal (COM5 Ch0)');
    xlabel('time (second)');
    ylabel('signal');
    subplot(2,2,2);
    %     plot(Time,Ch1(SignalLabelStart:SignalLabelEnd));
    %     line([Time(FinalSignalLabel(i)),Time(FinalSignalLabel(i))],[min(min(Ch0),min(Ch1)),max(max(Ch0),max(Ch1))],'Color','r');
    %     title('ch1');
    %     xlabel('time (second)');
    %     ylabel('signal');
    plot(EigenLabel,EigValue(2,VideoLabelStart:VideoLabelEnd));
    line([EigenLabel(i),EigenLabel(i)],[min(EigValue(2,:)),max(EigValue(2,:))],'Color','r');
